...
#### HTTPS 改善 HTTP 不安全問題的方法

- 機密性，HTTP 在通信過程中使用明文，這使得通信過程可能會被竊聽。為了防止被竊聽，最常見的處理方式就是加密。加密方式如下：

- 通信加密：HTTP 協議中沒有加密機制，HTTPS 則使用 SSL 或者 TSL 來進行加密，在過程中使用非對稱加密，利用公鑰跟私鑰來加密，因此就算 https 被中間人攔截，內容也是加密過的所以比較安全

- 身份認證，HTTP 協議中不會對請求和應答的雙方進行身份確認，所以身份有可能被中間人偽裝。HTTPS 的 SSL/TSL 可以去驗證證書，用來確定通信方是本人。證書是由值得信任的第三方機構辦法，所以只要能夠確認通信方持有的證書，則可以確認對方身份。

https://juejin.cn/post/6939183379238060069
