...
:W3C 規範中的一個概念，簡單來說具有 BFC 特性的元素可以看做是隔離了的獨立容器，容器裡面的元素不會被外面的元素所影響。
...
而 Flexbox 則是一個一維佈局系統，代表一次只能控制一個維度(ex, 行)，用於沿著單個軸（水平或垂直）排列項目。
它設計用於更靈活和簡單的佈局，不需要複雜的定位。Flexbox 尤其適用於創建需要根據不同的屏幕尺寸調整的響應式佈局，例如導航菜單、卡片佈局和可彈性調整的內容容器。
